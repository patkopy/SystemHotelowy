...
{
    public partial class LogowanieSprzatanie : Form
    {
        public LogowanieSprzatanie()
        {
            InitializeComponent();
        }

        private void LogowanieSprzatanie_Load(object sender, EventArgs e)
        {
        ...
        private void buttonZaloguj_Click(object sender, EventArgs e)
        {
            using (INZ2020HotelePKEntities db = new INZ2020HotelePKEntities())
            {
                ...
                var query = (from Uzytkownicy u in db.Uzytkownicy
                             ...
                             where (u.Login == login && u.Haslo == haslo && p.Stanowisko == "Administrator") || (u.Login == login && u.Haslo == haslo && p.Stanowisko == "Sprzątanie")
                             select u.IdUzytkownika).ToList();

                if (query.Count > 0)
                {
                    Hide();
                    SprzatanieForm sprzatanie = new SprzatanieForm(textBoxLogin.Text);
                    sprzatanie.Show();
                }
                else
