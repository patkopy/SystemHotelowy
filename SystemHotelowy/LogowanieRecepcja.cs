...
{
    public partial class LogowanieRecepcja : Form
    {
        public LogowanieRecepcja()
        {
            InitializeComponent();
        }

        private void LogowanieRecepcja_Load(object sender, EventArgs e)
        {
        ...
        private void buttonZaloguj_Click(object sender, EventArgs e)
        {
            using (INZ2020HotelePKEntities db = new INZ2020HotelePKEntities())
            {
                ...
                var query = (from Uzytkownicy u in db.Uzytkownicy
                             ...
                             where (u.Login == login && u.Haslo == haslo && p.Stanowisko == "Administrator") || (u.Login == login && u.Haslo == haslo && p.Stanowisko == "Recepcja")
                             select u.IdUzytkownika).ToList();

                if (query.Count > 0)
                {
                    Hide();
                    MenuRecepcji menu = new MenuRecepcji(textBoxLogin.Text);
                    menu.Show();
                }
                else
